...
PresentLogs := JeffGui.Add("Text", "x35 y200 w240 h20 c96d0ff +Center", "Presents: " prensentprize)
...
JeffGui.Add("Text", "x1140 y420 w140 h20 cffffff +BackgroundTrans", "Present multiplier : ")
global PresentMultiplier := JeffGui.Add("Edit", "x1290 y420 w80 h20 c000000", "100")
...
updateCurrentMap(nums) {
    MapLogs.Value := maps[nums][3]
}

updateWin(status := false) {
    global wins, totalMatch, prensentprize
    totalMatch++
    if (status) {
        prensentprize += Round(1500+1500*(Integer(PresentMultiplier.Value)/100))
        wins++
    }
    PresentLogs.Value := "Presents: " prensentprize
